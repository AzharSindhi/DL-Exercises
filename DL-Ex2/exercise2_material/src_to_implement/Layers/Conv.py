        ...
        for m in range(batch_size):
            for c in range(self.num_kernels):
                for d in range(input_depth):
                    output[m, c] += signal.correlate(input_tensor[m, d], self.weights[c,d], "same")
        ...
        for m in range(self.last_input.shape[0]):
            for i in range(self.num_kernels):
                for j in range(input_depth):
                    ...
                    upsampled_error = self.upsample_stride(error_tensor[m, i], self.last_input.shape[2:])
                    out_w = signal.correlate(padded_input[m, j], upsampled_error, "valid")
                    self.gradient_weights[i,j] += out_w

                    out_err = signal.convolve(upsampled_error, self.weights[i,j], "same")

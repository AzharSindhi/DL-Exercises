...
class ResNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.mean([2,3]) # global avarage pooling
        # x = torch.flatten(x,1)
        x = self.fc(x)
        out = self.sigmoid(x)
...
class ResBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.conv1x1 = torch.nn.Conv2d(self.in_channels, self.out_channels, 1, stride=self.stride, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        # print("reshaped identity:", identity.shape )
